...
impl ManualModule {
    ...
    pub unsafe fn load(bytes: &[u8]) -> Result<Self, String> {
        unsafe {
                        ...
                        while *int.add(i) != 0 {
                            ...
                            let proc_addr = if is_ordinal {
                                ...
                                if name_str == "LoadMenuA" {
                                    Some(proxy_LoadMenuA as *const () as usize as *mut c_void)
                                } else if name_str == "LoadCursorA" {
                                    Some(proxy_LoadCursorA as *const () as usize as *mut c_void)
                                } else if name_str == "ImageList_LoadImageA" {
                                    Some(
                                        proxy_ImageList_LoadImageA as *const () as usize
                                            as *mut c_void,
                                    )
                                } else {
            ...
            if let Err(e) = crate::patch::channel::apply(&module_info) {
                log::error!("Failed to apply Channel Server patches: {}", e);
            }
            if let Err(e) = crate::patch::sound_patch::apply(&module_info) {
            ...
            unsafe {
                crate::patch::loader_hook::OCX_MODULE = Some(windows::Win32::Foundation::HMODULE(
