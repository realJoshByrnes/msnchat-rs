...
use crate::config::{MSNConfigManager, MSNConfig};
use uuid::Uuid;
...
type RegCreateKeyExAType = unsafe extern "system" fn(usize, *const i8, u32, *const i8, u32, u32, *mut c_void, *mut usize, *mut u32) -> i32;
type RegCloseKeyType = unsafe extern "system" fn(usize) -> i32;
type RegQueryValueExAType = unsafe extern "system" fn(usize, *const i8, *mut u32, *mut u32, *mut u8, *mut u32) -> i32;
type RegSetValueExAType = unsafe extern "system" fn(usize, *const i8, u32, u32, *const u8, u32) -> i32;
...
type RegQueryInfoKeyAType = unsafe extern "system" fn(usize, *mut i8, *mut u32, *mut u32, *mut u32, *mut u32, *mut u32, *mut u32, *mut u32, *mut u32, *mut u32, *mut c_void) -> i32;
type RegEnumValueAType = unsafe extern "system" fn(usize, u32, *mut i8, *mut u32, *mut u32, *mut u32, *mut u8, *mut u32) -> i32;
type RegEnumKeyExAType = unsafe extern "system" fn(usize, u32, *mut i8, *mut u32, *mut u32, *mut i8, *mut u32, *mut c_void) -> i32;
...
pub unsafe fn apply(info: &super::module_info::ModuleInfo) -> Result<(), String> {
    ...
    let apis = [
        ("RegOpenKeyExA", std::ptr::addr_of_mut!(O_REG_OPEN_KEY_EX_A) as *mut c_void, detour_reg_open_key_ex_a as *mut c_void),
        ("RegCreateKeyExA", std::ptr::addr_of_mut!(O_REG_CREATE_KEY_EX_A) as *mut c_void, detour_reg_create_key_ex_a as *mut c_void),
        ("RegCloseKey", std::ptr::addr_of_mut!(O_REG_CLOSE_KEY) as *mut c_void, detour_reg_close_key as *mut c_void),
        ("RegQueryValueExA", std::ptr::addr_of_mut!(O_REG_QUERY_VALUE_EX_A) as *mut c_void, detour_reg_query_value_ex_a as *mut c_void),
        ("RegSetValueExA", std::ptr::addr_of_mut!(O_REG_SET_VALUE_EX_A) as *mut c_void, detour_reg_set_value_ex_a as *mut c_void),
        ("RegDeleteKeyA", std::ptr::addr_of_mut!(O_REG_DELETE_KEY_A) as *mut c_void, detour_reg_delete_key_a as *mut c_void),
        ("RegDeleteValueA", std::ptr::addr_of_mut!(O_REG_DELETE_VALUE_A) as *mut c_void, detour_reg_delete_value_a as *mut c_void),
        ("RegQueryInfoKeyA", std::ptr::addr_of_mut!(O_REG_QUERY_INFO_KEY_A) as *mut c_void, detour_reg_query_info_key_a as *mut c_void),
        ("RegEnumValueA", std::ptr::addr_of_mut!(O_REG_ENUM_VALUE_A) as *mut c_void, detour_reg_enum_value_a as *mut c_void),
        ("RegEnumKeyExA", std::ptr::addr_of_mut!(O_REG_ENUM_KEY_EX_A) as *mut c_void, detour_reg_enum_key_ex_a as *mut c_void),
    ];

    for (name, trampoline, detour) in apis {
        let proc = unsafe { GetProcAddress(advapi32, PCSTR::from_raw(CString::new(name).unwrap().as_ptr() as *const u8)) };
        if let Some(p) = proc {
...
unsafe extern "system" fn detour_reg_open_key_ex_a(
    ...
    let subkey_str = if lp_subkey.is_null() { "" } else { CStr::from_ptr(lp_subkey).to_str().unwrap_or("") };
    let path = resolve_path(hkey, Some(subkey_str));

    if let Some(p) = &path {
        if is_virtual_path(p) {
            let v_hkey = NEXT_VIRTUAL_HANDLE.fetch_add(1, Ordering::SeqCst);
            log::info!("Virtualizing RegOpenKeyExA -> handle: {:#x} path: {}", v_hkey, p);
...
unsafe extern "system" fn detour_reg_create_key_ex_a(
    ...
    if let Some(p) = &path {
        if is_virtual_path(p) {
            let v_hkey = NEXT_VIRTUAL_HANDLE.fetch_add(1, Ordering::SeqCst);
            log::info!("Virtualizing RegCreateKeyExA -> handle: {:#x} path: {}", v_hkey, p);
            VIRTUAL_HANDLES.lock().unwrap().insert(v_hkey, p.clone());
            unsafe { *phk_result = v_hkey };
    ...
    if let Some(orig) = unsafe { O_REG_CREATE_KEY_EX_A } {
        unsafe { orig(hkey, lp_subkey, reserved, lp_class, dw_options, sam_desired, lp_security_attributes, phk_result, lp_dw_disposition) }
    } else {
        2 // ERROR_FILE_NOT_FOUND
    }
}
...
unsafe extern "system" fn detour_reg_query_value_ex_a(
    ...
    let val_name = if lp_value_name.is_null() { "" } else { CStr::from_ptr(lp_value_name).to_str().unwrap_or("") };
    let path = resolve_path(hkey, None);

    if let Some(p) = &path {
        if is_virtual_path(p) {
            log::info!("Virtualizing RegQueryValueExA -> path: {} value: {}", p, val_name);
            ...
            if p_lower.ends_with("msnchat\\4.0") {
                ...
                if let Ok(config) = manager.load() {
                    if val_name.eq_ignore_ascii_case("UserData1") {
                        ...
                        if !lp_type.is_null() { unsafe { *lp_type = 4 }; } // REG_DWORD
                        if !lpcb_data.is_null() {
                            unsafe { *lpcb_data = 4 };
                            if !lp_data.is_null() {
            ...
            if p_lower.contains("appevents\\schemes\\apps\\chatocx") {
                ...
                let default_wav = match *event_id {
                    "msnchat_Whisper" | "msnchat_HostWhisper" | "msnchat_TagWhisper" => "ChatWhsp.wav",
            ...
            if p_lower.ends_with("appevents\\schemes") && val_name.is_empty() {
                let bytes = b".Current";
                if !lp_type.is_null() { unsafe { *lp_type = 1 }; }
                if !lpcb_data.is_null() {
                    let max_len = unsafe { *lpcb_data } as usize;
                    unsafe { *lpcb_data = 9 };
            ...
            if p_lower.contains("windows\\currentversion") && val_name.eq_ignore_ascii_case("MediaPath") {
            ...
            if p_lower.contains("clsid\\{f58e1cef") || p_lower.contains("clsid\\{fa980e7e") {
                ...
                if val_name.eq_ignore_ascii_case("{E113C6A6-D44A-4639-A40E-3B6DE32A1A40}") {
                    ...
                    if let Ok(bytes) = hex::decode(&guid_str) {
                        if !lp_type.is_null() { unsafe { *lp_type = 3 }; } // REG_BINARY
                        if !lpcb_data.is_null() {
                            unsafe { *lpcb_data = 16 };
                            if !lp_data.is_null() {
                                unsafe { std::ptr::copy_nonoverlapping(bytes.as_ptr(), lp_data, 16) };
                            }
                        }
                        return 0;
                    }
            ...
            if p_lower.contains("activex compatibility") && val_name.eq_ignore_ascii_case("Compatibility Flags") {
                if !lp_type.is_null() { unsafe { *lp_type = 4 }; } // REG_DWORD
                if !lpcb_data.is_null() {
                    unsafe { *lpcb_data = 4 };
                    if !lp_data.is_null() {
    ...
    if let Some(orig) = unsafe { O_REG_QUERY_VALUE_EX_A } {
        unsafe { orig(hkey, lp_value_name, lp_reserved, lp_type, lp_data, lpcb_data) }
    } else {
        2 // ERROR_FILE_NOT_FOUND
    }
}
...
unsafe extern "system" fn detour_reg_set_value_ex_a(
    ...
    if let Some(p) = &path {
        if is_virtual_path(p) {
            log::info!("Virtualizing RegSetValueExA -> path: {} value: {}", p, val_name);
            let p_lower = p.to_lowercase();
            let manager = MSNConfigManager::new(Path::new("config.toml"));
            ...
            if p_lower.ends_with("msnchat\\4.0") {
                if val_name.eq_ignore_ascii_case("UserData1") {
                    ...
                    let token = String::from_utf8_lossy(bytes).trim_end_matches('\0').to_string();
            ...
            if p_lower.contains("appevents\\schemes\\apps\\chatocx") {
                ...
                let event_id = parts.iter().rev().nth(1).unwrap_or(&"");
                let bytes = unsafe { std::slice::from_raw_parts(lp_data, cb_data as usize) };
                let wav = String::from_utf8_lossy(bytes).trim_end_matches('\0').to_string();
                log::info!("Sound Scheme write bypassed for event: {}, wav: {}", event_id, wav);
                return 0;
            }
...
unsafe extern "system" fn detour_reg_delete_value_a(hkey: usize, lp_value_name: *const i8) -> i32 {
    ...
    if let Some(p) = &path {
        if is_virtual_path(p) {
            log::info!("Virtualizing RegDeleteValueA -> path: {} value: {}", p, val_name);
            let p_lower = p.to_lowercase();
            if p_lower.ends_with("msnchat\\4.0") {
                let manager = MSNConfigManager::new(Path::new("config.toml"));
...
unsafe extern "system" fn detour_reg_query_info_key_a(
    ...
    if let Some(p) = &path {
        if is_virtual_path(p) {
            ...
            if !lpc_subkeys.is_null() { unsafe { *lpc_subkeys = 0 }; }
            if !lpc_values.is_null() { unsafe { *lpc_values = 0 }; }
            return 0;
        }
    }
...
unsafe extern "system" fn detour_reg_enum_value_a(
    ...
    if let Some(p) = &path {
        if is_virtual_path(p) {
            log::info!("Virtualizing RegEnumValueA -> path: {} index: {}", p, dw_index);
            let p_lower = p.to_lowercase();
            if p_lower.contains("resdllinstalled") {
                let manager = MSNConfigManager::new(Path::new("config.toml"));
                let config = manager.load().unwrap_or_default();
                ...
                if (dw_index as usize) < dlls.len() {
                    ...
                    if max_name_len >= len + 1 {
                        unsafe {
                            std::ptr::copy_nonoverlapping(bytes.as_ptr(), lp_value_name as *mut u8, len);
    ...
    if let Some(orig) = unsafe { O_REG_ENUM_VALUE_A } {
        unsafe { orig(hkey, dw_index, lp_value_name, lpcch_value_name, lp_reserved, lp_type, lp_data, lpcb_data) }
    } else {
        6 // ERROR_INVALID_HANDLE
    }
}
...
unsafe extern "system" fn detour_reg_enum_key_ex_a(
    ...
    if let Some(p) = &path {
        if is_virtual_path(p) {
            log::info!("Virtualizing RegEnumKeyExA -> path: {} index: {}", p, dw_index);
            return 259; // ERROR_NO_MORE_ITEMS
        }
    }

    if let Some(orig) = unsafe { O_REG_ENUM_KEY_EX_A } {
        unsafe { orig(hkey, dw_index, lp_name, lpcch_name, lp_reserved, lp_class, lpcch_class, lpft_last_write_time) }
    } else {
        6 // ERROR_INVALID_HANDLE
    }
}
